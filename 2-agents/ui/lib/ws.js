...
const connect = (url) => {
  ...
  ws.onopen = () => {
    ...
    if (currentSubscription !== null) {
      ws.send(JSON.stringify({ type: "chat.subscribe", conversationId: currentSubscription }));
    }
    emit("open");
...
// 订阅某个 conversationId 的事件流(null = 订阅全部)。
// 重连后会自动重订阅。
const subscribe = (conversationId) => {
  currentSubscription = conversationId == null ? null : Number(conversationId);
  send({ type: "chat.subscribe", conversationId: currentSubscription });
};
