...
export function createMindmap({ viewport, world, svg, topics, save, ui }: {
    ...
    for (const m of vis) {
      ...
      if (!el) {
        ...
        el.addEventListener('contextmenu', (e) => {
          e.preventDefault(); e.stopPropagation(); longPress.cancel(); endDrag(); select(id)
          const topic = rowOf(id)
          if (topic) ui.onContextMenu?.({ ...topic }, e.clientX, e.clientY)
        })
  ...
  function beginEdit(id, initial = null) {
    ...
    input.rows = 1
    input.setAttribute('aria-label', '编辑节点')
    input.value = initial ?? row.text
    input.maxLength = 300
    stop(input)
    input.addEventListener('keydown', (e) => {
      e.stopPropagation()
      if (e.isComposing || e.target?.closest?.('[role="dialog"], [role="menu"], [data-ui]')) return // 中文输入法组词中的 Enter/Esc 不处理
    ...
    function resizeEditor() {
      if (editing !== id) return
      const style = getComputedStyle(input), box = getComputedStyle(el)
      measure.font = style.font
      const textWidth = Math.max(0, ...input.value.split('\n').map((line) => measure.measureText(line || ' ').width))
      const frame = parseFloat(box.paddingLeft) + parseFloat(box.paddingRight) + parseFloat(box.borderLeftWidth) + parseFloat(box.borderRightWidth)
      el.style.width = `${Math.min(parseFloat(box.maxWidth) || 420, Math.max(96, Math.ceil(textWidth + frame + 8)))}px`
      input.style.height = '0px'
      input.style.height = `${Math.min(240, input.scrollHeight)}px`
      syncView() // 用编辑框的新尺寸重排连线与相邻节点，保留输入框和光标。
    }
  ...
  let sorting = false
  function siblingPosition(direction) {
    const current = rowOf(sel)
    if (!current?.parent_id) return null
    const parent = rowOf(current.parent_id), side = meta.get(current.id)?.side
    const list = childrenOf(parent.id).filter((t) => parent.parent_id || (meta.get(t.id)?.side || t.side) === side)
    const to = list.indexOf(current) + direction
    return to >= 0 && to < list.length ? current : null
  }
  async function moveSibling(direction) {
    const current = siblingPosition(direction)
    if (!current || sorting) return
    sorting = true
    try {
      const saved = await save.reorder(current.id, direction)
      for (const topic of saved) { const row = rowOf(topic.id); if (row) row.sort_order = topic.sort_order }
      syncView()
    } catch (error) { ui.onError?.('排序保存失败：' + error.message) }
    finally { sorting = false }
  }
  async function placeSibling(id, target, placement) {
    if (sorting) { syncView(); return }
    sorting = true
    try {
      const saved = await save.place(id, target, placement)
      for (const topic of saved) { const row = rowOf(topic.id); if (row) row.sort_order = topic.sort_order }
    } catch (error) { ui.onError?.('排序保存失败：' + error.message) }
    finally { sorting = false; syncView() }
  }
  ...
  world.appendChild(insertionLine)
  // 同级节点上下边缘是插入区，中间仍然可以接收子节点。使用拖前坐标避免目标跟着动画跳动。
  function siblingDropAt(w) {
    const current = rowOf(drag.id)
    const side = meta.get(current.id)?.side
    let nearest = null, distance = Infinity
    for (const topic of childrenOf(current.parent_id)) {
      if (topic.id === current.id || meta.get(topic.id)?.side !== side) continue
      const c = drag.anchors.get(topic.id), size = meas.get(topic.id)
      if (!c || !size || Math.abs(w.x - c.x) > size.w / 2 + 24) continue
      for (const placement of ['before', 'after']) {
        const y = c.y + (placement === 'before' ? -1 : 1) * (size.h / 2 + 8)
        const d = Math.abs(w.y - y)
        if (d <= 18 && d < distance) { distance = d; nearest = { target: topic.id, placement, x: c.x, y, width: size.w + 16 } }
      }
    }
    return nearest
  }
  function markInsertion(drop) {
    insertionLine.hidden = !drop
    if (!drop) return
    insertionLine.style.left = `${drop.x - drop.width / 2}px`
    insertionLine.style.top = `${drop.y}px`
    insertionLine.style.width = `${drop.width}px`
  }
  ...
  const longPress = createLongPress((id, x, y) => {
    const topic = rowOf(id)
    if (!topic || editing === id) return
    endDrag(); select(id)
    ui.onContextMenu?.({ ...topic }, x, y)
  })
  function trackTouch(e) {
    if (e.pointerType === 'touch' || e.pointerType === 'pen') touching.add(e.pointerId)
    if (touching.size > 1) longPress.cancel()
  }
  function releaseTouch(e) { touching.delete(e.pointerId); longPress.end(e) }
  window.addEventListener('pointerdown', trackTouch, true)
  window.addEventListener('pointermove', longPress.move, true)
  window.addEventListener('pointerup', releaseTouch, true)
  window.addEventListener('pointercancel', releaseTouch, true)
  window.addEventListener('blur', longPress.cancel)

  function onNodePointerDown(e, id) {
    e.stopPropagation()
    select(id)
    const row = rowOf(id)
    if (editing !== id && touching.size === 1) longPress.start(id, e)
    if (!row?.parent_id || editing === id || e.button > 0 || sorting) return // 根主题挪不动,编辑中不拖
    const el = els.get(id)
    const sub = subtreeIds(id)
    const base = new Map()
    for (const kid of sub) { const c = cur.get(kid); if (c) base.set(kid, { ...c }) }
    drag = { id, pointerId: e.pointerId, sub, base, anchors: new Map([...cur].map(([key, value]) => [key, { ...value }])), insert: null, sx: e.clientX, sy: e.clientY, target: null, wx: 0, moved: false }
  ...
  function onNodePointerMove(e) {
    ...
    drag.insert = siblingDropAt(w)
    markInsertion(drag.insert)
    const next = drag.insert ? null : dropTargetAt(w)
    if (next !== drag.target) {
      markTarget(next)
      if (next) {
  ...
  function onNodePointerUp(e) {
    ...
    if (finished.insert) { void placeSibling(finished.id, finished.insert.target, finished.insert.placement); return }
    // 空白处没有结构落点，回到原位置。
    if (!finished.target) { syncView(); return }
  ...
  function onKeydown(e) {
    if (e.isComposing || e.target?.closest?.('[role="dialog"], [role="menu"], [data-ui]')) return
  ...
  return {
    openMenu() {
      const topic = rowOf(sel), el = els.get(sel)
      if (!topic || !el) return
      const rect = el.getBoundingClientRect()
      ui.onContextMenu?.({ ...topic }, rect.left + rect.width / 2, rect.bottom + 8)
    },
    moveSibling,
    canMoveSibling: (direction) => Boolean(siblingPosition(direction)),
    appendTopics(parentId, topics) {
      const parent = rowOf(parentId)
      if (!parent) return
      parent.collapsed = 0
      for (const topic of topics) if (!rowOf(topic.id)) rows.push(topic)
      syncView()
    },
    ...
    destroy() {
      longPress.cancel()
      window.removeEventListener('pointerdown', trackTouch, true)
      window.removeEventListener('pointermove', longPress.move, true)
      window.removeEventListener('pointerup', releaseTouch, true)
      window.removeEventListener('pointercancel', releaseTouch, true)
      window.removeEventListener('blur', longPress.cancel)
