...
export function saveAdapter(mapId: number): SaveAdapter {
    ...
    return {
        ...
        async place(id, target, placement) {
            const response = await fetch('/api/reorder', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ map: mapId, topic: id, target, placement }) });
            const result = await response.json();
            if (!response.ok) throw new Error(result.error || '排序保存失败');
            return result.topics;
        },
        async reorder(id, direction) {
            const response = await fetch('/api/reorder', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ map: mapId, topic: id, direction }) });
            const result = await response.json();
            if (!response.ok) throw new Error(result.error || '排序保存失败');
            return result.topics;
        },
