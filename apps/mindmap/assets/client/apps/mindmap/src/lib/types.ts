...
export interface SaveAdapter {
    ...
    place(id: number, target: number, placement: 'before' | 'after'): Promise<Topic[]>;
    reorder(id: number, direction: number): Promise<Topic[]>;
    remove(id: number): Promise<void>;
}

/** 引擎把状态推出来,外壳负责显示 —— 引擎不认识 React。 */
export interface MindmapUI {
    onContextMenu?(topic: Topic, x: number, y: number): void;
...
export interface MindmapEngine {
    openMenu(): void;
    moveSibling(direction: number): void;
    canMoveSibling(direction: number): boolean;
    appendTopics(parentId: number, topics: Topic[]): void;
    addChild(): void;
    addSibling(): void;
    removeSelected(): void;
    /** 根主题删不得 */
