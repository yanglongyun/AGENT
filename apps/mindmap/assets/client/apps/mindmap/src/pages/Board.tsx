...
export function Board({ mapId, onBack }: { mapId: number; onBack: () => void }) {
    ...
    const openMenu = useCallback((topic: Topic, x: number, y: number) => setMenu({ topic, x, y }), []);
    ...
    const { refs, engine, zoom, count, selected, canRemove } = useMindmapEngine(mapId, topics, notify, mode === 'canvas', openMenu);
    ...
    return (
        <div className="board">
            ...
            {menu && <div className="node-menu-mask" onPointerDown={() => setMenu(null)} onContextMenu={(event) => { event.preventDefault(); setMenu(null); }}>
                <div className="node-menu" role="menu" aria-label="节点操作" style={{ left: Math.max(8, Math.min(menu.x, window.innerWidth - 190)), top: Math.max(8, Math.min(menu.y, window.innerHeight - 145)) }}>
                    <button autoFocus role="menuitem" onKeyDown={(event) => { if (event.key === 'Escape') setMenu(null); }} onPointerDown={(event) => event.stopPropagation()} onClick={() => { setTarget(menu.topic); setMenu(null); }}>生成子节点</button>
                    <button role="menuitem" disabled={!engine.current?.canMoveSibling(-1)} onPointerDown={(event) => event.stopPropagation()} onClick={() => { engine.current?.moveSibling(-1); setMenu(null); }}>上移 <span>⌥ ↑</span></button>
                    <button role="menuitem" disabled={!engine.current?.canMoveSibling(1)} onPointerDown={(event) => event.stopPropagation()} onClick={() => { engine.current?.moveSibling(1); setMenu(null); }}>下移 <span>⌥ ↓</span></button>
                </div>
            </div>}
            {target && <GenerateChildren mapId={mapId} topic={target} onClose={() => setTarget(null)} onGenerated={(added) => {
                engine.current?.appendTopics(target.id, added);
                notify(`已生成 ${added.length} 个子节点`);
                setTarget(null);
            }} />}
