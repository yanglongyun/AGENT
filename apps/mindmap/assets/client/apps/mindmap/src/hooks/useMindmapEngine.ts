 ...
 */
export function useMindmapEngine(mapId: number, topics: Topic[] | null, onError: (message: string) => void, enabled = true, onContextMenu?: (topic: Topic, x: number, y: number) => void) {
    ...
    useEffect(() => {
        ...
        engine.current = createMindmap({
            ...
            ui: { onSelection: setSelected, onZoom: setZoom, onCount: setCount, onError, onContextMenu },
        });
    ...
    }, [mapId, topics, onError, enabled, onContextMenu]);
