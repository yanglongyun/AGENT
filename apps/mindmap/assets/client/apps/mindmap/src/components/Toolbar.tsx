...
export function Toolbar({ zoom, canEdit, canRemove, on }: {
    ...
    useLayoutEffect(() => {
        const toolbar = ref.current, container = toolbar?.parentElement;
        if (!toolbar || !container) return;
        const fit = () => {
            toolbar.classList.remove('compact');
            toolbar.classList.toggle('compact', toolbar.scrollWidth > toolbar.clientWidth);
        };
        fit();
    ...
    return (
        <div className="toolbar" ref={ref} data-ui>
            ...
            <button type="button" title="节点操作" aria-label="节点操作" disabled={!canEdit} onClick={on.menu}>
                <svg viewBox="0 0 24 24"><circle cx="5" cy="12" r="1" /><circle cx="12" cy="12" r="1" /><circle cx="19" cy="12" r="1" /></svg><span>操作</span>
            </button>
            <i />
