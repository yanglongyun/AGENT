...
（脚本用 `import.meta.url` 定位自身）。在 AGENT 项目根目录操作宿主这份数据时使用：

```bash
APP_DATA_DIR="$PWD/.data/apps/mindmap" node apps/mindmap/scripts/mindmap.mjs map list
```

后续 CLI 操作都带上同一个 `APP_DATA_DIR`。支持的子命令：
...
AGENT 为本应用固定使用 `.data/apps/mindmap/`，重启后数据仍在。独立运行使用另一份默认数据目录。结构：
...
画布节点右键菜单支持「生成子节点」「上移」「下移」。选中节点也可用 Alt/Option + ↑/↓ 调整同侧同级顺序。排序一次保存所有相关位置，兼容旧数据的重复序号。

生成窗口可指定 1–10 个子节点与补充要求。应用服务通过宿主 `/host/ai/complete` 调用模型，需要 `ai.complete` 权限；每次生成记录到宿主任务列表。结果追加到原节点，失败显示错误，不插入空节点。

- `POST /api/generate`：`{ "map": 1, "topic": 1, "count": 3, "instruction": "从三个角度展开" }`，返回 `{ topics, task }`。
- `POST /api/reorder`：`{ "map": 1, "topic": 2, "direction": -1 }`，方向为 `-1`（上移）或 `1`（下移），返回排序后的 `{ topics }`。根节点不能调序。

拖动节点到同级节点的上边缘或下边缘，出现蓝色插入线后松开，即可调整同级位置；拖到其他节点中央则移动为它的子节点。子树一起移动。拖到空白处会归位，导图仍按层级自动布局。

`POST /api/reorder` 也接受 `{ "map": 1, "topic": 2, "target": 3, "placement": "before" }`，`placement` 为 `before` 或 `after`，插入到同侧同级目标的前或后。

节点编辑框随内容自动伸缩，最大宽度 420px，超过后自动换行并增加高度。Enter 保存，Shift + Enter 换行，Escape 取消本次编辑；中文输入法确认候选词不会触发保存。

移动端长按节点约半秒可打开操作菜单，也可先点选节点，再点击工具栏「⋯ / 节点操作」。窄屏菜单从底部展开。拖动、抬手、触摸取消或多指操作会取消长按计时。
