...
function reorderTopic(input) {
  const mapId = Number(input.map), id = Number(input.topic), direction = Number(input.direction);
  if (input.target === undefined && direction !== -1 && direction !== 1) throw new Error('排序方向无效');
  const db = load(), topic = db.topics.find((topic) => topic.id === id && topic.map_id === mapId);
  if (!topic || topic.parent_id === null) throw new Error('根节点不能调整同级顺序');
  const parent = db.topics.find((node) => node.id === topic.parent_id);
  const siblings = db.topics.filter((node) => node.parent_id === parent.id && (parent.parent_id !== null || node.side === topic.side))
    .sort((a, b) => a.sort_order - b.sort_order || a.created_at - b.created_at || a.id - b.id);
  const from = siblings.indexOf(topic);
  let to = from + direction;
  if (input.target !== undefined) {
    const target = siblings.find((node) => node.id === Number(input.target));
    if (!target || target.id === id || !['before', 'after'].includes(input.placement)) throw new Error('只能拖到同侧同级节点的前后');
    siblings.splice(from, 1);
    to = siblings.indexOf(target) + (input.placement === 'after' ? 1 : 0);
    siblings.splice(to, 0, topic);
  } else if (to >= 0 && to < siblings.length) {
    [siblings[from], siblings[to]] = [siblings[to], siblings[from]];
  }
  if (to >= 0 && to < siblings.length) {
    const stamp = now();
    siblings.forEach((node, index) => { node.sort_order = index; node.updated_at = stamp; });
    db.maps.find((map) => map.id === mapId).updated_at = stamp;
    save(db);
  }
  return { topics: siblings };
}

const generating = new Set();
async function generateChildren(input, signal) {
  const mapId = Number(input.map), id = Number(input.topic), count = Number(input.count ?? 3);
  const instruction = String(input.instruction || '').trim();
  if (!Number.isInteger(count) || count < 1 || count > 10) throw new Error('生成数量必须是 1 到 10');
  if (instruction.length > 2000) throw new Error('生成要求不能超过 2000 字');
  if (!process.env.HOST_URL || !process.env.APP_TOKEN) throw new Error('请在已配置模型的宿主中使用生成子节点');
  const key = `${mapId}:${id}`;
  if (generating.has(key)) throw new Error('这个节点正在生成，请稍候');
  const before = load();
  const parent = before.topics.find((topic) => topic.id === id && topic.map_id === mapId);
  if (!parent) throw new Error('节点不存在');
  const path = [], seen = new Set();
  for (let node = parent; node && !seen.has(node.id); node = before.topics.find((topic) => topic.id === node.parent_id)) {
    seen.add(node.id); path.unshift(node.text);
  }
  const existing = before.topics.filter((topic) => topic.parent_id === id).map((topic) => topic.text);
  generating.add(key);
  try {
    const response = await fetch(`${process.env.HOST_URL.replace(/\/+$/, '')}/host/ai/complete`, {
      method: 'POST', signal,
      headers: { 'content-type': 'application/json', authorization: `Bearer ${process.env.APP_TOKEN}` },
      body: JSON.stringify({
        title: `导图：为「${parent.text}」生成子节点`,
        instructions: '你负责扩展思维导图。根据主题路径、已有子节点与用户要求，生成简洁、具体、互不重复的直属子节点。使用主题的语言，不重复已有内容。只返回符合 schema 的 JSON。',
        prompt: JSON.stringify({ path, existing, count, instruction }),
        schemaName: 'mindmap_children',
        schema: { type: 'object', additionalProperties: false, required: ['children'], properties: {
          children: { type: 'array', minItems: count, maxItems: count, items: { type: 'string' } },
        } },
      }),
    });
    const result = await response.json();
    if (!response.ok || result.error) throw new Error(result.error || '生成失败');
    let children;
    try { children = JSON.parse(result.text).children; } catch { throw new Error('模型未返回有效的子节点'); }
    if (!Array.isArray(children) || children.length !== count || children.some((text) => typeof text !== 'string' || !text.trim() || text.trim().length > 300)) throw new Error('模型返回的子节点数量或内容无效');
    children = children.map((text) => text.trim());
    if (new Set(children).size !== count) throw new Error('模型返回了重复子节点');
    if (signal.aborted) throw new Error('生成已取消');
    // 等待模型期间其他节点可能被编辑，提交前重读，不能用旧快照覆盖整张导图。
    const db = load(), current = db.topics.find((topic) => topic.id === id && topic.map_id === mapId);
    if (!current) throw new Error('节点已删除，生成结果未保存');
    if (current.text !== parent.text) throw new Error('节点内容已修改，请重新生成');
    const siblings = db.topics.filter((topic) => topic.parent_id === id);
    if (children.some((text) => siblings.some((topic) => topic.text.trim() === text))) throw new Error('生成结果与已有子节点重复，请重试');
    const stamp = now(), start = Math.max(-1, ...siblings.map((topic) => topic.sort_order)) + 1;
    const added = children.map((text, index) => ({ id: db.nextTopicId++, map_id: mapId, parent_id: id, text,
      side: current.side || 'right', sort_order: start + index, collapsed: 0, created_at: stamp, updated_at: stamp }));
    db.topics.push(...added); current.collapsed = 0; current.updated_at = stamp;
    db.maps.find((map) => map.id === mapId).updated_at = stamp;
    save(db);
    return { topics: added, task: result.task };
...
function serve() {
  ...
  createServer(async (req, res) => {
    try {
      ...
      if (pathname === '/api/reorder' && req.method === 'POST') return send(res, 200, JSON.stringify(reorderTopic(await body(req))));
      if (pathname === '/api/generate' && req.method === 'POST') {
        const controller = new AbortController();
        const cancel = () => controller.abort();
        res.on('close', cancel);
        try { return send(res, 200, JSON.stringify(await generateChildren(await body(req), controller.signal))); }
