...
test('提议按聊天隔离，规则原子追加、忽略无副作用、重复处理拒绝，随聊天删除', (t) => {
    ...
    const payload = { kind: 'rule', summary: '建议', detail: '原因', text: '新增规则', old_text: '已有规则', new_text: '已有规则\n\n新增规则' };
    ...
    assert.equal(createStore(db).listProposals(a.id)[0].new_text, '已有规则\n\n新增规则');
...
test('规则提议支持修改、删除、全部替换，冲突保留待处理且不覆盖后来修改', (t) => {
    const { store } = setup(t);
    const chat = store.createThread({ title: '编辑规则', rules: '中文回答\n保持简短' });
    const propose = (old_text, new_text, replace_all = false) => store.createProposal(chat.id, { kind: 'rule', summary: '调整规则', detail: '编辑', old_text, new_text, replace_all });
    const edit = propose('中文回答', '英文回答');
    assert.equal(store.getThread(chat.id).rules, '中文回答\n保持简短');
    store.setRules(chat.id, '中文回答\n保留后来增加的规则');
    store.answerProposal(chat.id, edit.id, 'accept');
    assert.equal(store.getThread(chat.id).rules, '英文回答\n保留后来增加的规则');
    const conflict = propose('英文回答', '中文回答');
    store.setRules(chat.id, '日文回答');
    assert.throws(() => store.answerProposal(chat.id, conflict.id, 'accept'), /冲突/);
    assert.equal(store.listProposals(chat.id)[0].id, conflict.id);
    assert.equal(store.getThread(chat.id).rules, '日文回答');
    const remove = propose('日文回答', '');
    store.answerProposal(chat.id, remove.id, 'accept');
    assert.equal(store.getThread(chat.id).rules, '');
    const create = propose('', '规则');
    store.setRules(chat.id, '新的规则');
    assert.throws(() => store.answerProposal(chat.id, create.id, 'accept'), /规则已变化/);
    store.setRules(chat.id, '重复\n重复');
    assert.throws(() => propose('重复', '内容'), /出现 2 次/);
    const all = propose('重复', '$&', true);
    store.answerProposal(chat.id, all.id, 'accept');
    assert.equal(store.getThread(chat.id).rules, '$&\n$&');
    store.setRules(chat.id, '甲\r\n乙');
    const crlf = propose('甲\n乙', '甲\n丙');
    store.answerProposal(chat.id, crlf.id, 'accept');
    assert.equal(store.getThread(chat.id).rules, '甲\r\n丙');
});
