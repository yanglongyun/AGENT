...
import { injection, normalizeRule } from '../../agent/rules.js';
...
export function createRuns({ config, store, files, approvals, broadcast }) {
    ...
    async function work(conversation, user, controller, runtime) {
        ...
        try {
            const result = await runAgent({
                ...
                // 审批门。ask 由这里实现 —— 弹卡、等表态;没人回应就超时当拒绝
                permission: {
    ...
    return {
        ...
        start(conversation, content, attachments = [], clientId = '') {
            ...
            const mode = conversation.permission_mode || savedSettings.permissionMode || config.rule.defaultMode || 'ask';
            const rules = store.listRules(conversation.id).map(normalizeRule).filter((rule) => rule.enabled);
            const runtime = {
                ...config,
                ...
                // 规矩进 instructions —— 每轮重新组装、独立于消息历史,压缩吃不掉它
                instructions: [savedSettings.instructions || '', injection(rules, mode)].filter(Boolean).join('\n\n'),
                permissionMode: mode,
                rules,
            };
