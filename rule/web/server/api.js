...
import { describe } from '../../agent/danger.js';
import { MODES, injection, matches, normalizeRule } from '../../agent/rules.js';
...
export function createApi({ config, store, runs, files, channel, approvals, compileRule, meta }) {
        ...
        try {
            ...
            if (method === 'GET' && url.pathname === '/api/meta') {
                json(response, 200, {
                    ...
                    defaultMode: store.getSettings().permissionMode || config.rule.defaultMode || 'ask',
                });
                return true;
            }
            ...
            if (method === 'PUT' && url.pathname === '/api/settings') {
                ...
                const allowed = ['driver', 'responsesUrl', 'apiKey', 'model', 'instructions', 'permissionMode'];
            ...
            if (method === 'GET' && url.pathname === '/api/rules') {
                json(response, 200, { rules: store.listRules(url.searchParams.get('conversationId') || '') });
                return true;
            }
            if (method === 'POST' && url.pathname === '/api/rules') {
                const input = await readBody(request);
                const text = String(input.text || '').trim();
                if (!text) { json(response, 400, { error: '先写下你的规矩' }); return true; }
                // 编译在这里发生:大白话进去,拦截条件出来;编译不出来就只剩提示词那一个出口
                const compiled = input.match
                    ? { ...normalizeRule(input), text, compiled: true, note: '' }
                    : await compileRule(text, input.kind ? { kind: input.kind } : {});
                const rule = store.createRule({
                    ...normalizeRule({ ...compiled, conversationId: String(input.conversationId || '') }),
            ...
            if (method === 'GET' && url.pathname === '/api/rules/preview') {
                const conversationId = url.searchParams.get('conversationId') || '';
                const conversation = conversationId ? store.getConversation(conversationId) : null;
                const mode = conversation?.permission_mode || store.getSettings().permissionMode || config.rule.defaultMode || 'ask';
                const rules = store.listRules(conversationId).map(normalizeRule).filter((rule) => rule.enabled);
                json(response, 200, { mode, text: injection(rules, mode) });
                return true;
            }
            if (method === 'POST' && url.pathname === '/api/rules/reorder') {
                const input = await readBody(request);
            ...
            if (method === 'POST' && url.pathname === '/api/rules/test') {
                // 自检:粘一条命令进来,当场看命中不命中。规矩的效力必须当场可验
                const input = await readBody(request);
                const rule = normalizeRule(input.rule || {});
                const requestShape = describe(String(input.tool || 'bash'), {
                    command: String(input.command || ''),
                    path: String(input.path || ''),
                });
                const workdir = normalizeWorkdir(input.workdir) || config.workdir;
                json(response, 200, {
                    hit: matches(rule, requestShape, { home: homedir(), cwd: workdir }),
            ...
            if (segments[1] === 'rules' && segments[2]) {
                ...
                if (method === 'PATCH') {
                    ...
                    if (!current) { json(response, 404, { error: '规矩不存在' }); return true; }
                    ...
                    if (text && text !== current.text) {
                        const result = await compileRule(text, input.kind ? { kind: input.kind } : {});
