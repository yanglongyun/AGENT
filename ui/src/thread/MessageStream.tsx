...
export function MessageStream({ readOnly = false }: { readOnly?: boolean }) {
    const { rows, busy, ready, hasMore, loadingOlder, tick, viewSeq, currentId } = useThread();
    ...
    return (
        <main ref={scrollRef} className="stream" onScroll={onScroll}>
            ...
            <div key={currentId || '__draft__'} ref={innerRef} className="stream-inner">
                ...
                {ready && !rows.length && readOnly && <span className="chip">任务暂无消息</span>}
                {!ready && !rows.length && <span className="chip">正在打开对话…</span>}

                {ready && !rows.length && !readOnly && (
