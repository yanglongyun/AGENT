...
export function createCompiler({ config, store }) {
     ...
     *
     * 问句必须是人话。早先直接把 `动作=install;工具=bash;路径=` 丢过去,
     * 模型看不懂那是什么就一律答 no —— 正确的编译被成批打回,
     * 每条规则都降级成「只靠自觉」,比没有这道校验还糟。
     */
    async function verify(base, text, match) {
        const parts = [];
    ...
    /**
     * @returns { text, prompt, match, compiled, note }
     *   compiled=false 表示这条规则拦不住任何东西,只写进了提示词。
     */
    return async function compileRule(text) {
        const raw = String(text || '').trim();
        const bare = { text: raw, prompt: raw, match: { tools: [], actions: [], paths: [] }, compiled: false, note: '' };
        if (!raw) return { ...bare, note: '内容为空' };

        const base = runtime();
        if (!base) return { ...bare, note: '还没配置模型,这条只能靠助理自觉遵守' };

        const result = await complete({ ...base, instructions: INSTRUCTIONS, input: [{ role: 'user', content: raw }] })
            .catch(() => ({ text: '' }));
