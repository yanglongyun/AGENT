...
const polishNote = async ({ content = "", mode = "polish" }) => {
  ...
  const result = await callLlmStream(settings.provider, settings.apiUrl, settings.apiKey, {
    ...
    messages: [
      { role: "system", content: "你是 Agent 记事本的写作助手。你的任务是处理用户当前笔记草稿,不保存数据,不执行工具。" },
