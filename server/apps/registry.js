...
export function createApps({ config, broadcast = () => {} }) {
    const root = resolve(config.appsDir);
    ...
    function readOne(id) {
        ...
        if (raw.run && asString(raw.run.command)) {
            ...
            manifest.run = {
                ...
                mode,
                idleTimeoutMs: Number(config.appIdleTimeoutMs) || 600_000,
            };
    ...
    return {
        ...
        promptSection() {
            ...
            const lines = apps.map((app) => `- ${app.id}(${app.name}):${app.description || '无说明'} —— API 见 ${config.appsDir}/${app.id}/APP.md`);
            const broken = scan().filter((app) => app.invalid);
            if (broken.length) lines.push(...broken.map((app) => `- ${app.id}:【故障】${app.invalid}`));
            if (!lines.length) return '';
            const hostBase = `http://${config.host}:${config.port}`;
            return [
                '## 已安装的应用',
                ...
                `新建 app:在 ${config.appsDir}/<id>/ 下按仓库根 SPEC.md 的契约建目录,`,
