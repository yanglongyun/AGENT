...
export function createRuns({ config, store, files, approvals, apps, broadcast }) {
    ...
    return {
        ...
        start(conversation, content, attachments = [], clientId = '') {
            ...
            const runtime = {
                ...
                // 规则进 instructions —— 每轮重新组装、独立于消息历史,压缩吃不掉它
                // 规则 + 已装应用清单,都进 instructions —— 每轮重新组装,压缩吃不掉
                instructions: [savedSettings.instructions || '', injection(rules, mode), apps?.promptSection() || '']
                    .filter(Boolean).join('\n\n'),
                permissionMode: mode,
                rules,
                consult,
