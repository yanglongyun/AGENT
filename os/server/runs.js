...
export function createRuns({ config, store, files, apps, broadcast }) {
    ...
    return {
        ...
        start(conversation, content, attachments = [], clientId = '') {
            ...
            const runtime = {
                ...
                // app 清单常驻提示词:AI 得先知道装了什么,才谈得上用它或改它
                instructions: [savedSettings.instructions || '', apps?.promptSection() || ''].filter(Boolean).join('\n\n'),
            };
