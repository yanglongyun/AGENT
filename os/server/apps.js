// app 注册表:扫描 os/apps/*、读 manifest、校验、给 AI 拼常驻清单。
//
...
export function createApps({ config, broadcast = () => {} }) {
    const root = resolve(config.os.appsDir);
    ...
    function readOne(id) {
        ...
        if (raw.server && asString(raw.server.command)) {
            manifest.server = {
                ...
                idleTimeoutMs: Number(raw.server.idleTimeoutMs) || Number(config.os.appIdleTimeoutMs) || DEFAULT_IDLE_MS,
            };
        }
    ...
    return {
        ...
        promptSection() {
            ...
            const lines = apps.map((app) => `- ${app.id}(${app.name}):${app.description || '无说明'} —— 细节见 ${config.os.appsDir}/${app.id}/APP.md`);
            ...
            return [
                ...
                ...lines,
                '',
                `新建 app:在 ${config.os.appsDir}/<id>/ 下建 manifest.json、APP.md、src/ 和构建产物 dist/index.html,`,
