...
import { cycleTheme, useTheme, type ThemeMode } from '../lib/theme';
...
export function Sidebar() {
    ...
    const row = (conversation: Conversation) => (
        <div
            key={conversation.id}
            className={`conv${conversation.id === currentId ? ' on' : ''}`}
    ...
    return (
        <>
            ...
            <aside className={`sidebar${shell.collapsed ? ' folded' : ''}${shell.drawer ? ' open' : ''}`}>
                ...
                <button
                    className="side-new"
                    onClick={() => { shell.closeDrawer(); createDraft(); }}
                >
                ...
                <div className="side-foot">
                    <button
                        className="icon-btn"
                        title={THEME_META[themeMode].label}
                        onClick={cycleTheme}
                    >
                        <Icon name={THEME_META[themeMode].icon} size={15} />
                    </button>
                    <span className="side-meta clip">本地运行{meta.version ? ` · v${meta.version}` : ''}</span>
                </div>
            </aside>
