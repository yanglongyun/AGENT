...
export function createRuns({ config, store, files, broadcast }) {
    ...
    async function autoTitle(conversationId, userContent, items) {
    ...
    async function work(conversation, user, controller) {
        ...
        try {
            ...
            if (conversation.title === DEFAULT_TITLE) void autoTitle(conversationId, user.content, result.items);
        } catch (error) {
    ...
    return {
        ...
        start(conversation, content, attachments = [], clientId = '') {
            ...
            work(conversation, user, controller).catch((error) => {
