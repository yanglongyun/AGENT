...
export function shell({
    ...
    if (typeof executable !== 'string' || !executable) throw new Error('shell.executable 必填');
    if (!Array.isArray(args) || args.some((item) => typeof item !== 'string')) throw new Error('shell.args 必须是字符串数组');
    positive('shell.minTimeoutMs', minTimeoutMs);
    positive('shell.defaultTimeoutMs', defaultTimeoutMs);
    positive('shell.maxTimeoutMs', maxTimeoutMs);
    positive('shell.maxOutputChars', maxOutputChars);
    if (minTimeoutMs > defaultTimeoutMs || defaultTimeoutMs > maxTimeoutMs) throw new Error('shell 超时参数顺序非法');

    return (input = {}, context = {}) => {
        const command = String(input.command || '');
        if (!command) return Promise.resolve({ exit_code: -1, stdout: '', stderr: 'command 不能为空' });
        if (typeof context.cwd !== 'string' || !context.cwd) throw new Error('shell 执行需要 context.cwd');
